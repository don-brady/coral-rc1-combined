	...
	const uint64_t spare, const uint64_t children)
{
	...
	static const uint64_t bases11[1][11] = {{1, 4, 5, 9, 3, 2, 8, 10, 7, 6, 0}};
	static const uint64_t bases19[1][19] = {{1, 5, 6, 11, 17, 9, 7, 16, 4, 10, 12, 3, 15, 18, 14, 13, 8, 2, 0,}};
	static const uint64_t bases23[1][23] = {{1, 8, 18, 6, 2, 16, 13, 12, 4, 9, 3, 10, 11, 19, 14, 20, 22, 15, 5, 17, 21, 7, 0,}};
	static const uint64_t bases31[1][31] = {{1, 8, 2, 16, 4, 17, 12, 3, 24, 6, 10, 18, 20, 5, 9, 15, 27, 30, 23, 29, 7, 25, 14, 19, 28, 26, 22, 21, 13, 11, 0,}};
		...
		const uint64_t spare, const uint64_t children, const char *path)
{
	...
	if (cfg == NULL) {
		...
		if (cfg == NULL) {
			fprintf(stderr, "Cannot create supported configuration\n");
			return (1);
		}
		freecfg = B_TRUE;
	}
...
{
	printf(gettext("draidcfg [-r] [-n children] [-d data] [-p parity] [-s spare] <configfile>\n"));
	return (1);
}
...
main(int argc, char **argv)
{
	...
	if (argc < 1) {
		fprintf(stderr, gettext("Missing configuration file argument\n"));
		return (usage());
	}
	...
	if (data == 0 || parity == 0 || spare == 0 || children == 0) {
		fprintf(stderr, gettext("Missing data/parity/spare/children argument\n"));
		return (usage());
	}
