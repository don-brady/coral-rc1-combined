...
typedef struct 
{
        ...
        int **rows;                             /* each row maps all drives, groups from 0, spares down from ndevs-1 */
...
static void permute_devs (int *in, int *out, int ndevs) {
        ...
        for (i = 0; i < ndevs; i++) {           /* assign random order */
                tmp[i].value = in[i];
                tmp[i].order = mrand48();       /* should be hash generated from params */
        }
...
static map_t *dup_map (map_t *oldmap) 
{
...
static map_t *new_map (int groupsz, int ngroups, int nspares, int nrows) 
{
        map_t *map = malloc(sizeof(map_t));
...
static inline int is_broken (map_t *map, int dev) 
{
        int i;
...
static double eval_decluster (map_t *map, int how, int faults, int print)
{
        ...
        switch (how) {
        case EVAL_WORST:                        /* imbalance from worst possible drive failure */
                val = max_ios;                  /* => insensitive to failures handled better  */
                break;
        case EVAL_MEAN:                         /* average over all possible drive failures */
                val = ((double)sum)/n;          /* => sensitive to all possible failures */
                break;
        case EVAL_RMS:                          /* root mean square over all possible drive failures */
                val = sqrt(((double)sumsq)/n);  /* => penalizes higher imbalance more */
                break;
        default:
                ASSERT(0);
        }
...
static int rand_in_range(int min, int count) 
{
        return (min + drand48()*count);
}

static void permute_map (map_t *map, int temp) {
        static int prev_temp;
        
        int nrows = (temp < 1) ? 1 :
                    (temp > 100) ? map->nrows : rand_in_range(1, (map->nrows * temp)/100);
        int row   = rand_in_range(0, map->nrows - nrows);
        int ncols = map->ndevs; //rand_in_range(map->groupsz * 2, (map->ndevs * temp)/100);
...
static map_t *develop_map (map_t *map) 
{
        map_t *dmap = new_map (map->groupsz, map->ngroups, map->nspares, map->nrows *map->ndevs);
        int    base;
        int    dev;
        int    i;
...
static map_t *optimize_map (map_t *map, int eval, int faults) {
        double temp    = 100.0;
        double alpha   = 0.995;
...
static void print_map_stats (map_t *map, int optimize, int print_ios) 
{
	double score = eval_decluster(map, EVAL_WORST, 1, 0);
        ...
        printf("%6s (%2d x %2d + %2d) x %5d: %2.3f\n",
               ...
               map->ngroups, map->groupsz, map->nspares, map->nrows,
	       score);
...
int draid_permutation_generate(struct vdev_draid_configuration *cfg)
{
	...
	for (i = 0, best_map = NULL; i < loop; i++) {
		...
		if (rc != sizeof(seed)) {
			printf("Not enough entropy at /dev/random: read %d, wanted %lu.\n",
			       rc, sizeof(seed));
		...
		if (eval_decluster(omap, eval, faults, 0) > eval_decluster(map, eval, faults, 0)) {
			/* optimize_map() may create a worse map, because the simulated annealing
			 * process may accept worse neighbors to avoid getting stuck in local optima
			 */
			free_map(omap);
		} else {
			free_map(map);
		...
		if (best_map == NULL ||
		    eval_decluster(map, eval, faults, 0) < eval_decluster(best_map, eval, faults, 0)) {
			if (best_map != NULL)
				free_map(best_map);
			best_map = map;
			best_seed = seed;
	...
	close(urand_fd);
	if (i != loop)
		fprintf(stderr, "Early termination at loop %d. Generated permutations may not be optimal!\n",
			i + 1);
