    ...
    const struct vdev_draid_configuration *cfg, uint64_t **array)
{
	...
	for (c = 0; c < mm->mm_children; c++) {
		...
		mc->mc_vd = zio->io_vd->vdev_child[permutation[idx]];
		mc->mc_offset = o;
	}
...
vdev_draid_get_astart(const vdev_t *vd, const uint64_t start, boolean_t mirror)
{
	...
	uint64_t ms_id = start >> vd->vdev_ms_shift;
...
static boolean_t
vdev_draid_io_mirrored(zio_t *zio)
{
	const vdev_t *vd = zio->io_vd;
	...
	mirror = DVA_GET_DRAID_MIRROR(dva) != 0 ? B_TRUE : B_FALSE;
...
vdev_draid_asize(vdev_t *vd, uint64_t psize, uint64_t offset)
{
	...
	mirror = vdev_draid_ms_mirrored(vd, offset >> vd->vdev_ms_shift);
